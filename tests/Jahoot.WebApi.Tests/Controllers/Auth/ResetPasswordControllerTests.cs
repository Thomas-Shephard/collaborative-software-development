...
public class ResetPasswordControllerTests
{
    ...
    public async Task ResetPassword_ValidRequest_ReturnsOk()
    {
        ...
        _mockUserRepository.Setup(r => r.GetUserByEmailAsync(requestModel.Email)).ReturnsAsync(user);
        ...
        _mockPasswordResetRepository.Setup(r => r.GetPasswordResetTokenByEmail(requestModel.Email)).ReturnsAsync(passwordResetToken);
        ...
        _mockUserRepository.Verify(r => r.UpdateUserAsync(It.Is<User>(u => u.PasswordHash != "old_hash")), Times.Once);
        _mockPasswordResetRepository.Verify(r => r.UpdatePasswordResetTokenAsync(It.Is<PasswordResetToken>(t => t.IsUsed)), Times.Once);
    ...
    public async Task ResetPassword_UserNotFound_ReturnsBadRequest()
    {
        ...
        _mockUserRepository.Setup(r => r.GetUserByEmailAsync(requestModel.Email)).ReturnsAsync((User?)null);
    ...
    public async Task ResetPassword_PasswordResetTokenNotFound_ReturnsBadRequest()
    {
        ...
        User user = new() { UserId = 1, Email = requestModel.Email, Name = "Test User", PasswordHash = "old_hash", Roles = new List<Role> { Role.Student } };
        _mockUserRepository.Setup(r => r.GetUserByEmailAsync(requestModel.Email)).ReturnsAsync(user);
        _mockPasswordResetRepository.Setup(r => r.GetPasswordResetTokenByEmail(requestModel.Email)).ReturnsAsync((PasswordResetToken?)null);
    ...
    public async Task ResetPassword_IncorrectTokenHash_ReturnsBadRequest()
    {
        ...
        {
            Email = "test@example.com",
            Token = "incorrect_token", // This token won't match the hashed one
            NewPassword = "NewSecurePassword123"
        };
    ...
    public void ResetPassword_ExceptionDuringTransaction_RollsBackAndThrows()
    {
        ...
        _mockUserRepository.Setup(r => r.UpdateUserAsync(It.IsAny<User>())).ThrowsAsync(new InvalidOperationException("Simulated DB error"));
