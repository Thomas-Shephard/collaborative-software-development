...
public class ForgotPasswordControllerTests
{
    ...
    public async Task ForgotPassword_UserFound_CreatesTokenAndSendsEmailAndReturnsOk()
    {
        ...
        _emailServiceMock.Setup(emailService => emailService.SendEmailAsync(user.Email, "Jahoot Password Reset", It.IsAny<string>()))
            .Callback<string, string, string>((_, _, body) => capturedEmailBody = body)
            .Returns(Task.CompletedTask);
        ...
        // Extract token from email body "Use the code {token}  to reset your password."
        string plainToken = capturedEmailBody!.Split("Use the code ")[1].Split("  to reset your password.")[0];
        ...
        _emailServiceMock.Verify(s => s.SendEmailAsync(user.Email, "Jahoot Password Reset", It.Is<string>(body => body.Contains(plainToken))), Times.Once);
    }
}
