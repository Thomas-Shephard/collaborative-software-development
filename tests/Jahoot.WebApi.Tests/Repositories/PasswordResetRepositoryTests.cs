...
public class PasswordResetRepositoryTests : RepositoryTestBase
{
    ...
    public async Task UpdatePasswordResetTokenAsync_SuccessfulUpdate_UpdatesToken()
    {
        ...
        PasswordResetToken? updatedToken = await Connection.QuerySingleOrDefaultAsync<PasswordResetToken>("SELECT * FROM PasswordResetToken WHERE token_id = @TokenId", new { TokenId = originalToken.TokenId });
