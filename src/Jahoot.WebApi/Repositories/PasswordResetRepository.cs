...
public class PasswordResetRepository(IDbConnection connection, IUserRepository userRepository) : IPasswordResetRepository
{
    ...
    public async Task<PasswordResetToken?> GetPasswordResetTokenByEmail(string email)
    {
        ...
        const string query = "SELECT token_id, user_id, token_hash, expiration, is_used, is_revoked, created_at FROM PasswordResetToken WHERE user_id = @UserId AND is_used = FALSE AND is_revoked = FALSE";
    ...
    {
        await connection.ExecuteAsync("UPDATE PasswordResetToken SET is_used = @IsUsed WHERE token_id = @TokenId", passwordResetToken);
    }
}
